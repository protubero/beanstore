...
A BeanStore entity class has to extend class `AbstractEntity`. It must have a no-arg constructor and expose its properties by setters and getters - as required by the Java Beans Spec. Deviating from the specification, it does not have to be serializable. At least not in the traditional Java way. We use Kryo to persist the transactions, hence all properties must be serializable by Kryo.
...
The BeanStore is designed as a store of immutable objects! I.e. all property values have to be immutable. Stored beans will throw an exception if you call a setter method. You can easily shoot yourself in the foot by changing field values of stored beans. BeanStore only restricts access per setter method.
